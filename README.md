`vmux` is a terminal multiplexer with out-of-the-box support for search, mouse-controlled scrollback, and i3-like keybindings. Imagine `tmux` with a smaller learning curve and more user-friendly defaults.
...
```
nix run github:aaronjanse/vmux
```

###### Package manager

[![Packages for vmux](https://repology.org/badge/vertical-allrepos/vmux.svg)](https://repology.org/project/vmux/versions)

###### Building from source
1. Install Golang
2. `go get github.com/malivvan/vmux`
3. Run `vmux` to launch the terminal multiplexer

To update `vmux`, run `go get -u github.com/malivvan/vmux`
...
vmux searches `XDG_CONFIG_HOME` to find its config. If it cannot, it writes a config to `~/.config/vmux/config.toml` upon the first run. Modifiers in shortcuts (e.g. `Alt`) are case-insensitive.

You can detect if you're running a script inside vmux by checking if `THREEMUX` is equal to `1`.
