...
import (
	...
	"github.com/malivvan/vmux/ecma48"
...
const BUG_REPORT_URL = "https://github.com/malivvan/vmux/issues"
...
func init() {
	...
	threemuxDir = path.Join(tmp, "vmux")
...
func main() {
	...
	case "new":
		...
		if len(os.Args) != 3 {
			fmt.Println("Usage: vmux new <name>")
			os.Exit(1)
		}
		sessionName := os.Args[2]
	...
	case "attach":
		...
		if len(os.Args) != 3 {
			fmt.Println("Usage: vmux attach <name>")
			os.Exit(1)
		}
		sessionName := os.Args[2]
		sessionInfo, found, err := findSession(sessionName)
		...
		if err != nil {
			...
			fmt.Printf("To manually kill this session, run `vmux kill %s`\n", sessionName)
			os.Exit(1)
		}
	case "kill":
		if len(os.Args) != 3 {
			fmt.Println("Usage: vmux kill <name>")
			os.Exit(1)
		}
		sessionName := os.Args[2]
		sessionInfo, found, err := findSession(sessionName)
		...
		if err != nil {
			...
			fmt.Println("1. Ensure there are no unwanted vmux processes running")
...
func refuseNesting() {
	fmt.Println("Refusing to run vmux inside itself.")
